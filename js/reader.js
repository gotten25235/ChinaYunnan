...
(() => {
  ...
  function create(config){
    ...
    function itemBody(p,day=null){
      ...
      if(p.type==='food')extra+=(p.priority?detailValue('旅程優先級',p.priority):'')+detailValue('口味',p.taste)+detailValue('用餐類型',p.meal);if(p.type==='shopping')extra+=(p.priority?detailValue('旅程優先級',p.priority):'')+detailValue('購物分類',p.group)+detailValue('類型',p.marketKind)+(p.socialRank?detailValue('問一問推薦',`第 ${p.socialRank} 名${p.recommendationShare!==undefined?` · ${p.recommendationShare}% 人推薦`:''}`):'');if(p.type==='night')extra+=detailValue('建議時段',p.time)+detailValue('適合',p.suitable)+detailValue('飯店 → 景點',hotelDistanceValue(p,d),true)+detailValue('交通',p.transport,true);if(p.type==='photo')extra+=detailValue('怎麼拍',p.shot,true)+detailValue('光線／時段',p.bestTime,true);
