...
(() => {
  ...
  function create({$,tripData,esc,networkProfile,journeySystem,markRendered,isDev=false}){
    ...
    const ROUND_STORAGE_KEY='yunnan-2026-sun-wish-rounds-v1';
    const milestones={
      1:'雲散一點，太陽收到你的願望了。',
      3:'晴天娃娃開心了，好天氣再靠近一點。',
      5:'大放晴！把這片陽光留給旅途。',
    ...
    const MILESTONE_STEPS=[1n,3n,5n,7n];

    function cycleStep(taps){
      return taps>0n?Number(((taps-1n)%CYCLE_LENGTH)+1n):0;
    }

    function nextMilestoneTap(taps){
      if(taps<=0n)return 1n;
      const cycleBase=((taps-1n)/CYCLE_LENGTH)*CYCLE_LENGTH;
      const next=MILESTONE_STEPS.map(step=>cycleBase+step).find(value=>value>taps);
      return next??(cycleBase+CYCLE_LENGTH+1n);
    }
    ...
    let rounds=readCounter(ROUND_STORAGE_KEY);
    let roundTaps=0n;
    ...
    const roundLabel=()=>card()?.querySelector('[data-sun-wish-rounds]');
    const statusLabel=()=>card()?.querySelector('.sun-wish__stats');
    const messageLabel=()=>card()?.querySelector('[data-sun-wish-message]');
    const hintLabel=()=>card()?.querySelector('[data-sun-wish-hint]');
    ...
    function updateStats(){
      const countNode=countLabel(),roundNode=roundLabel(),stats=statusLabel();
      if(countNode)countNode.textContent=String(count);
      if(roundNode)roundNode.textContent=String(rounds);
      if(stats)stats.setAttribute('aria-label',`已祈晴 ${count} 次，完成 ${rounds} 輪祈晴`);
    }

    function updateHint(){
      const hint=hintLabel();
      if(!hint)return;
      const next=nextMilestoneTap(roundTaps);
      hint.textContent=roundTaps===0n
        ?'點一下，開始新一輪祈晴'
        :`本輪 ${roundTaps} 次 · 下一個彩蛋：第 ${next} 次`;
    }
    ...
    function renderSun(){
      ...
      const tapsForWhiteout=roundTaps>=20n?1:Number(roundTaps)/20;
    ...
    function sampleSun(now){
      ...
      if(t===1){
        segment=null;
        if(active.kind==='fade')finishRound();
        else holdSun();
      }
      renderSun();
    }
    ...
    function sunWishMarkup(){
      return `<section class="sun-wish" id="sun-wish" aria-label="晴天娃娃祈晴互動區">
        ...
        <button class="sun-wish__button" type="button" data-sun-wish aria-label="點一下晴天娃娃祈晴" aria-describedby="sun-wish-message sun-wish-stats">
          <span class="sun-wish__float"><span class="sun-wish__figure">
            <svg class="sun-wish__doll" viewBox="0 0 88 116" aria-hidden="true" focusable="false">
              ...
              <g class="sun-wish__happy-face" fill="none" stroke="var(--sun-wish-detail)" stroke-width="1.7" stroke-linecap="round">
                <path d="M36 25Q39 19 42 25M48 25Q51 20 54 25M40 39Q45 43 50 38"/>
              </g>
        ...
        <div class="sun-wish__copy">
          <h3>一路放晴 ☀️</h3>
          <p id="sun-wish-message" data-sun-wish-message aria-live="polite">雲散一點，陽光多一點。</p>
          <p class="sun-wish__hint" data-sun-wish-hint>點一下，開始新一輪祈晴</p>
        </div>
        <div class="sun-wish__stats" id="sun-wish-stats" role="status" aria-live="polite" aria-atomic="true" aria-label="已祈晴 ${count} 次，完成 ${rounds} 輪祈晴">
          <span class="sun-wish__stat" title="已祈晴次數"><span aria-hidden="true">🙏</span><strong data-sun-wish-count>${count}</strong></span>
          <span class="sun-wish__stat" title="完成祈晴輪數"><span aria-hidden="true">☀️</span><strong data-sun-wish-rounds>${rounds}</strong></span>
        </div>
    ...
    function handleAction(actionButton){
      ...
      const el=card(),wishButton=button(),sunSymbol=sunFigure(),response=skyResponse(),message=messageLabel();
      ...
      count+=1n;
      roundTaps+=1n;
      const step=cycleStep(roundTaps);
      ...
      if(step===3||step===5||step===7){
        ...
        if(step>=5)el.classList.add('is-full-sun');
        if(step===7)el.classList.add('clouds-cleared');
        clearTimeout(effectTimer);
      ...
      el.style.setProperty('--push-distance',`${roundTaps>=30n?120:Number(roundTaps)*4}px`);
      if(message)message.textContent=milestones[step]||phrases[step]||(roundTaps>=20n?'整片天空都是你集來的陽光。':'繼續集氣，天空還能更亮。');
      ...
      playFeedback('sun',sunSymbol,[
        {transform:'scale(1)'},
        {transform:'translateY(-3px) scale(1.055)',offset:.35},
        {transform:'scale(1)'}
      ],440);
      playFeedback('sky',response,[
        {opacity:0},{opacity:.09,offset:.35},{opacity:0}
      ],650);
      return true;
    }
