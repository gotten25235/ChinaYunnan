...
(() => {
  ...
  function create(config){
    ...
    function sourceFooter(points){const providers=[...new Set(points.flatMap(p=>pointRecord(p)?.providers||[]))];return `<span>${networkProfile?.name?.(networkProfile.get())||'國際版'} · 優先順序：高德 → QWeather Grid → Open-Meteo${providers.length?` · 本次 ${esc(providers.map(providerLabel).join(' + '))}`:''}</span><span class="weather-source-links"><a href="${AMAP_SOURCE}" target="_blank" rel="noopener noreferrer">高德 ↗</a><a href="${QWEATHER_SOURCE}" target="_blank" rel="noopener noreferrer">QWeather Grid ↗</a><a href="${OPEN_METEO_SOURCE}" target="_blank" rel="noopener noreferrer">Open-Meteo ↗</a></span>`;}
    ...
    return {daySummaryPlaceholder,dayPanelPlaceholder,mapPlaceholder,hydrate,handleAction,start,refresh,cleanup,settingsHtml,isStorageAvailable:()=>storageAvailable};
  }
