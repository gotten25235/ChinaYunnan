...
(() => {
  ...
  function create(config){
    ...
    function sourceFooter(points){
      ...
      const devBlock=`<span class="weather-dev-source"><b>DEV ONLY</b><span>${esc(networkProfile?.name?.(networkProfile.get())||'國際版')} · 欄位優先順序：高德 → QWeather Grid → Open-Meteo</span><span>資料來源：<a href="${AMAP_SOURCE}" target="_blank" rel="noopener noreferrer">高德天氣 API ↗</a> <a href="${QWEATHER_API_SOURCE}" target="_blank" rel="noopener noreferrer">QWeather API ↗</a> <a href="${OPEN_METEO_SOURCE}" target="_blank" rel="noopener noreferrer">Open-Meteo API ↗</a> <a href="${OPEN_METEO_ELEVATION_SOURCE}" target="_blank" rel="noopener noreferrer">Open-Meteo Elevation API ↗</a></span><span>高德：即時 + 短期預報 · QWeather：即時 + 多日 / 逐時預報 · Open-Meteo：逐日 / 逐時預報，最長可到 16 天</span><span>海拔來源：Open-Meteo / Copernicus DEM 90 m · ${esc(elevationDebug)}</span></span>`;
