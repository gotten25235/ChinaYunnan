...
(async () => {
  ...
  function bindMainViewSwipeController(){
    ...
    const INTENT=9,RATIO=1.3,DISTANCE_RATIO=.28,FLICK_VELOCITY=.45,MOUSE_INTENT=11,MOUSE_RATIO=1.2,MOUSE_DISTANCE_RATIO=.2,MOUSE_FLICK_VELOCITY=.5,WHEEL_THRESHOLD=110,WHEEL_GAP=190;
    const touchBlockedSelector=['button:not(.credit-link)','input','select','textarea','label','summary','dialog','[contenteditable="true"]','.section-nav','.bottom-nav','.content-card','.journey-card','.compact-card','.utility-card','.content-grid','.compact-grid','.culture-grid','.itinerary-day-strip','.itinerary-timetable-frame','.itinerary-timetable-scroll','.day-story-strip','.route-list','.chips','.map-date-control','.map-date-rail','.map-explorer','#map','.leaflet-container','.map-strip-section','.map-place-strip','.map-strip-card','.map-feature-card','.nearby-panel','.location-controls'].join(',');
    const desktopBlockedSelector=['a','button','input','select','textarea','label','summary','dialog','[contenteditable="true"]','.section-nav','.bottom-nav','.rail','.map-date-control','.map-date-rail','.itinerary-timetable-frame','.itinerary-timetable-scroll','.day-story-strip','.route-list','.chips','.map-explorer','#map','.leaflet-container','.map-strip-section','.map-place-strip','.map-strip-card','.map-feature-card','.nearby-panel','.location-controls','[data-item]','[data-culture-story]'].join(',');
    let gesture=null,pointerGesture=null,swipeLayer=null,suppressClickUntil=0,wheelSum=0,wheelLastT=0,wheelLockUntil=0;
    const reducedMotion=()=>window.matchMedia('(prefers-reduced-motion: reduce)').matches,inSwipeRegion=target=>!!target.closest?.('.workspace,footer'),isTouchBlocked=target=>!!target.closest?.(document.documentElement.dataset.uiLayout==='desktop'?desktopBlockedSelector:touchBlockedSelector),isDesktopBlocked=target=>!!target.closest?.(desktopBlockedSelector),activeDialog=()=>!!document.querySelector('dialog[open]'),changedTouch=(event,id)=>[...event.changedTouches].find(t=>t.identifier===id),liveTouch=(event,id)=>[...event.touches].find(t=>t.identifier===id);
