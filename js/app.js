...
(async () => {
  ...
  function bindMainViewSwipeController(){
    ...
    const reducedMotion=()=>window.matchMedia('(prefers-reduced-motion: reduce)').matches,
      ...
      isTouchBlocked=target=>!!target.closest?.(document.documentElement.dataset.uiLayout==='desktop'?desktopBlockedSelector:touchBlockedSelector),isDesktopBlocked=target=>!!target.closest?.(desktopBlockedSelector),activeDialog=()=>!!document.querySelector('dialog[open]'),changedTouch=(event,id)=>[...event.changedTouches].find(t=>t.identifier===id),liveTouch=(event,id)=>[...event.touches].find(t=>t.identifier===id);
