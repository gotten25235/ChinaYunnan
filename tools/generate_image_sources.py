...
def generated_text():
    ...
    lines = [
        ...
        "",
        "圖片實體依用途分成 `images/food/`、`images/shopping/`、`images/hotels/`、`images/places/`、`images/pose/`、`images/airlines/`、`images/handbook/` 七類。",
        "",
        "目前共有 **%d** 筆 image records；**%d** 筆本地 WebP 已存在；**%d** 筆等待由 exact remote 同步。" % (len(rows), local_present, pending),
        "",
    ...
    lines += [
        ...
        "- 舊欄位 `src`、`remoteSrc`、`sourceImage`、`fallbackSrc`、`sourceUrl`、`originalUrl`、`originalSource`、`captureSource`、`capturePage`、`publicCaption`、`id` 都禁止重新引入。",
